...
export const ThemeToggle = ({ className = '' }) => {
  ...
  return (
    <button
      ...
      className={`group relative inline-flex h-9 w-9 sm:h-9 sm:w-16 items-center justify-between rounded-full p-1 transition-all duration-300 ease-out active:scale-90 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/40 border border-black/[0.08] dark:border-white/[0.12] bg-black/[0.04] dark:bg-white/[0.08] backdrop-blur-xl shadow-2xs ${className}`}
    >
      ...
      <span className="hidden sm:flex items-center justify-between w-full px-1 text-zinc-400 dark:text-zinc-500">
        ...
        <Moon className={`h-3 w-3 transition-colors ${isDark ? 'text-indigo-300' : ''}`} />
      </span>
